...
$op=$_GET['op'];
  ...
  $pdf->Cell(30,6,'OP',1,0,'C',1);
  $pdf->Cell(40,6,'CLIENTE',1,0,'C',1);
  $pdf->Cell(50,6,'PRODUCTO',1,0,'C',1);
  $pdf->Cell(70,6,'OBSERVACIONES',1,0,'C',1);
...
while ($row = mysqli_fetch_assoc($consulta)) { 
    $pdf->Cell(30,6,utf8_decode($row['Id_Orden_Produccion']),1,0,'C'); 
    $pdf->Cell(40,6,utf8_decode($row['Nombre']),1,0,'C'); 
    $pdf->Cell(50,6,utf8_decode($row['Producto']),1,0,'C');
    $pdf->Cell(70,6,utf8_decode($row['observaciones']),1,0,'C');
    $pdf->Cell(25,6,utf8_decode($row['Cantidad']),1,0,'C');
    $pdf->Cell(30,6,utf8_decode($row['Fecha_Pedido']),1,0,'C');
